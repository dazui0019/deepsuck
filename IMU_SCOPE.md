...
Use `imu_scope.py` to display IMU CSV data in an oscilloscope-like interactive window powered by PyQtGraph.

Examples:

```bash
uv run python imu_scope.py data.csv
uv run python imu_scope.py data.csv --y ax ay az
uv run python imu_scope.py data.csv --sample-rate 100
uv run python imu_scope.py data.csv --sample-period 0.005
uv run python imu_scope.py data.csv --split
uv run python imu_scope.py data.csv --highpass-cutoff 0.5
uv run python imu_scope.py data.csv --lowpass-cutoff 5
uv run python imu_scope.py data.csv --highpass-cutoff 0.5 --lowpass-cutoff 5
```
